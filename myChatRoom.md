...
如果说公钥加密的文件，只有私钥可以解密，那么私钥可不可以进行加密？如果可以，使用什么进行解密？

答案就是，**私钥也可以加密，而解密的是公钥**

这种用法通常用在数字签名或者是身份验证的情况
...
-   客户端向服务端发送**“TLS版本，支持的密码算法，随机数，临时公钥”**
    ...
    **在 TCP 连通的瞬间，客户端调用操作系统的安全随机接口（`RAND_bytes`），用系统采集的物理噪声作为原料，在内存里现打出一个 32 位的临时数字，直接塞进第一个网络包里发出去了**

    客户端生成了一对公私钥，然后把公钥发送给服务端

**第二步：Server Hello**

-   服务端向客户端发送“**选择的协议，选择加密算法，临时公钥**”
...
1.   客户端和服务端分别在自己的电脑内存中，随即产生一个只有自身知道的数字——**临时私钥**，两个数字完全不同，绝对不通过网络传输

2.   然后在客户端和服务端分别将自己生成的数字通过OPENSSL库，产生两个新的数字，也就是两个**临时公钥**，双方把自己的临时公钥发送给对方，也就是**密钥交换**

3.   客户端收到了服务端发过来的（F（客户端临时私钥）），然后把再把自己客户端的私钥加进去

     （F（服务端私钥+F（客户端临时私钥）））

4.   服务端执行和客户端一样的操作，就会发现这样算出来的两个结果是一眼各地，就叫**预主密钥**

5.   但是此时的预主密钥不能直接用，再把**“预主密钥” + “客户端的随机数” + “服务端的随机数”**进行标准化的哈希派生，最后的出来的结果才是真正用来加密聊天内容的**“会话密钥（Session Key）”**

     

     在这个过程中，虽然客户端和服务端的临时公钥是公开的，但是仍然得不到临时私钥所以肯定得不到最后的会话密钥

**第四步：身份验证**

为了防止攻击者伪装成服务器，此时需要使用公钥加密

服务端向客户端发送**数字证书（Certificate）**，里面有“域名，服务器公钥，证书有效期，CA数字签名”，客户端检查
