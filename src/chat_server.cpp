...
void ChatServer::on_connection(
    ...
    if (connection->connected()) {
        ...
        connection->send(
            "[system] connected to chatroom v8.2 "
            "(Reactor + groups + Redis + SQLite + file transfer + official Protobuf).\n"
...
void ChatServer::handle_file_begin_private(
    ...
    const std::string& filename_base64 = words[2];
    ...
    if (!fileutil::is_valid_transfer_token(token) ||
        !is_valid_username(target) ||
        !parse_uint64_value(words[3], file_size) ||
    ...
    upload.file_name =
        fileutil::sanitize_filename(filename);
    upload.expected_size = file_size;
    upload.sha256_hex = words[4];
    upload.recipients.push_back(target);

    if (!file_transfer_service_.begin_upload(
            token,
            upload.temp_path,
            error
        )) {
...
void ChatServer::handle_file_begin_group(
    ...
    upload.target = group_name;
    upload.group_id = group->id;
    upload.file_name =
        fileutil::sanitize_filename(filename);
    upload.expected_size = file_size;
    upload.sha256_hex = words[4];
    upload.recipients = std::move(recipients);
...
void ChatServer::handle_file_chunk(
    ...
    if (!file_transfer_service_.append_upload_chunk(
...
void ChatServer::handle_file_end(
    ...
    if (session.upload->received_size !=
        ...
        reject_file_upload(
            ...
            session,
            token,
            "uploaded byte count does not match declared size"
        );
        return;
    }
    ...
    if (!file_transfer_service_.finalize_upload(
        ...
        file_transfer_service_.remove_temp_file(
            upload.temp_path
        );
    ...
    metadata.set_transfer_token(upload.token);
    metadata.set_scope(upload.scope);
    ...
    if (upload.scope ==
        ...
        metadata.set_group_id(upload.group_id);
        metadata.set_group_name(upload.target);
    }

    metadata.set_file_name(upload.file_name);
    metadata.set_file_size(upload.expected_size);
    metadata.set_sha256_hex(upload.sha256_hex);
    ...
    if (!database_.add_file_transfer(
        ...
        std::filesystem::remove(
            file_transfer_service_.storage_root() /
                stored_relative_path,
            ignored
        );
...
void ChatServer::handle_file_received(
    ...
    if (!transfer) {
        session.file_deliveries_in_progress.erase(
            transfer_id
        );
        return;
    }
...
void ChatServer::handle_file_receive_failed(
    ...
    connection->send(
        "[system] file #F" +
        std::to_string(transfer_id) +
        " remains pending; use PENDING to retry.\n"
    );
}
...
void ChatServer::deliver_file_to_user(
    ...
    if (!session->file_deliveries_in_progress
             .insert(transfer.id)
             .second) {
        return;
    }
    ...
    file_transfer_service_.deliver_async(
        ...
        [
            ...
            if (success) {
                // Network output is drained, but durable delivery is
                // acknowledged only after chat_client writes the file,
                // verifies SHA-256, and sends FILE_RECEIVED.
                return;
            }
...
void ChatServer::abort_active_upload(
    ClientSession& session
) {
    if (!session.upload) {
        return;
