...
```c
        ...
        }//当然，对于叶子结点也是可行的，右为0，递归左，左也为0，返回1
...
1.  当一个节点**只有一个孩子**时（比如只有左或只有右），它就像走在悬崖边的独木桥上。此时，**唯一的活路**就是那个非空的孩子。
    -  **代码动作**：你的 `if` 判断捕捉到这个情况，**直接丢弃空的那一边**，根本不把 `0` 拿来比较，强行递归唯一的那个孩子。
    -  **作用**：保证递归路线**永远沿着存在的节点走**，不会因为 `0` 而提前终止在非叶子节点上。
2.  当一个节点**左右两个孩子都健在**时（比如根节点 `3`），此时才真正进入“求最小”的环节。
    -  **代码动作**：两个 `if` 都没拦住，执行 `return min(minDepth(left), minDepth(right)) + 1;`
    -  **作用**：这行代码是**整台机器的引擎**。它会分别派两个人（左信使和右信使）去左右两边的子树探路，等两人回来报告深度后，**只采纳数字较小的那个**（较浅的子树），然后加上当前这一层（`+1`）
3.  当一个节点**左右都是空**（即叶子节点）时，游戏结束。
    -  **代码动作**：由于 `root->right == nullptr` 成立，程序不会去比较，而是直接 `return minDepth(root->left) + 1;`，而 `minDepth(nullptr)` 返回 `0`，所以叶子节点返回 `0 + 1 = 1`。
    -  **作用**：**这就是“地基”**。所有深度的计数都从这里开始往上垒。
